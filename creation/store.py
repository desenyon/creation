...
from creation.config import CONFIG_DIR, ensure_dirs

DB_PATH = CONFIG_DIR / "creation.db"


def _conn() -> sqlite3.Connection:
    """Local-first SQLite connection.

    Creation runs entirely on the user's device — one file at ``~/.creation/creation.db``,
    no server, no cloud. WAL mode + a busy timeout let the always-on dispatcher loop
    and the local web server read/write concurrently without "database is locked".
    """
    ensure_dirs()
    # timeout: wait up to 30s for a competing writer instead of erroring immediately.
    conn = sqlite3.connect(DB_PATH, timeout=30.0)
    conn.row_factory = sqlite3.Row
    try:
        conn.execute("PRAGMA journal_mode=WAL")  # concurrent readers + one writer
        conn.execute("PRAGMA synchronous=NORMAL")  # durable enough locally, much faster
        conn.execute("PRAGMA busy_timeout=30000")
    except sqlite3.OperationalError:
        pass  # e.g. read-only FS / unusual mounts — fall back to defaults
    return conn
...
def init_db() -> None:
    with _conn() as c:
        ...
        for col, typ in [
            ("current_phase", "TEXT DEFAULT ''"),
            ("agent_log", "TEXT DEFAULT ''"),
            # Work-graph linkage (non-breaking; null for legacy build runs).
            ("ticket_id", "TEXT"),
            ("agent_def_id", "TEXT"),
            ("org_id", "TEXT DEFAULT 'local'"),
            ("team_id", "TEXT"),
            ...
            try:
                ...
                pass
        for col, typ in [("template_id", "TEXT DEFAULT 'greenfield'"), ("max_turn_budget", "INTEGER")]:
            try:
...
class Run:
    ...
    agent_def_id: Optional[str] = None
    org_id: str = "local"
...
def create_project(
    ...
    with _conn() as c:
        c.execute(
            "INSERT INTO projects (id,name,idea,status,workdir,agent,template_id,max_turn_budget,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (pid, name, idea, "draft", workdir, agent, template_id, max_turn_budget, ts, ts),
        )
    return Project(
        id=pid,
        name=name,
...
def delete_project(pid: str, *, remove_files: bool = True) -> bool:
    ...
    if not p:
        return False
    with _conn() as c:
        c.execute("DELETE FROM run_queue WHERE project_id=?", (pid,))
...
def update_project(pid: str, **kwargs: Any) -> Optional[Project]:
    ...
    with _conn() as c:
        c.execute(
            "UPDATE projects SET name=?, idea=?, status=?, agent=?, template_id=?, workdir=?, max_turn_budget=?, updated_at=? WHERE id=?",
            (p.name, p.idea, p.status, p.agent, p.template_id, p.workdir, p.max_turn_budget, p.updated_at, pid),
        )
    return p
...
def count_running_runs() -> int:
    init_db()
    with _conn() as c:
        r = c.execute("SELECT COUNT(*) AS n FROM runs WHERE status='running'").fetchone()
...
def list_running_runs() -> List[Dict[str, Any]]:
    ...
    with _conn() as c:
        rows = c.execute(
            """
            SELECT r.id AS run_id, r.project_id, r.created_at, p.name AS project_name, p.agent
            FROM runs r
...
def get_running_run_for_project(project_id: str) -> Optional[str]:
    init_db()
    with _conn() as c:
        r = c.execute(
            "SELECT id FROM runs WHERE project_id=? AND status='running' ORDER BY created_at DESC LIMIT 1",
            (project_id,),
        ).fetchone()
...
def get_running_project_id() -> Optional[str]:
    """First running project (legacy). Prefer list_running_runs()."""
    runs = list_running_runs()
    return str(runs[0]["project_id"]) if runs else None
...
def _row_project(r: sqlite3.Row) -> Project:
    ...
    d.setdefault("max_turn_budget", None)
